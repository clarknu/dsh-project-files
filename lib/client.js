...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var HTML_EXT = new Set(['html', 'htm'])
    var TXT_EXT = new Set(['txt', 'md', 'markdown', 'json', 'js', 'ts', 'jsx', 'tsx', 'mjs', 'cjs', 'py', 'rb', 'go', 'java', 'c', 'h', 'cpp', 'hpp', 'cs', 'rs', 'php', 'sh', 'bash', 'zsh', 'bat', 'ps1', 'yaml', 'yml', 'toml', 'ini', 'cfg', 'conf', 'xml', 'css', 'scss', 'less', 'sql', 'csv', 'tsv', 'log', 'env', 'gitignore', 'dockerfile', 'makefile', 'vue', 'svelte'])
    ...
    // Bare-URL autolink: split plain text into strings + <a> elements.
    // Trailing ASCII/Chinese sentence punctuation is not swallowed into href.
    function autolinkText(s, key) {
      var out = [], re = /(^|[\s(])(https?:\/\/[^\s<)]+)/g, last = 0, m
      while ((m = re.exec(s))) {
        var ust = m.index + m[1].length, url = m[2], trail = ''
        while (url.length && /[.,;:!?\u3002]/.test(url.charAt(url.length - 1))) { trail = url.charAt(url.length - 1) + trail; url = url.slice(0, -1) }
        if (ust > last) out.push(s.slice(last, ust))
        out.push(h('a', { key: key + '_u' + out.length, href: url }, url))
    ...
    function renderInline(s, key) {
      ...
      var last = 0, m
      while ((m = re.exec(s))) {
        if (m.index > last) parts.push.apply(parts, autolinkText(s.slice(last, m.index), key))
        var tok = m[0], k = key + '__' + parts.length
        if (tok.charAt(0) === '`') parts.push(h('code', { key: k }, tok.slice(1, -1)))
        else if (tok.slice(0, 2) === '**') parts.push(h('strong', { key: k }, tok.slice(2, -2)))
        else if (tok.slice(0, 2) === '~~') parts.push(h('del', { key: k }, tok.slice(2, -2)))
        else if (tok.charAt(0) === '*') parts.push(h('em', { key: k }, tok.slice(1, -1)))
        else if (tok.slice(0, 2) === '![') { var im = /^!\[([^\]]*)\]\(([^)]+)\)$/.exec(tok); parts.push(h('img', { key: k, src: im[2], alt: im[1] })) }
        else if (tok.charAt(0) === '<') { var au = tok.slice(1, -1); parts.push(h('a', { key: k, href: au }, au)) }
        else { var lk = /^\[([^\]]+)\]\(([^)]+)\)$/.exec(tok); parts.push(h('a', { key: k, href: lk[2] }, lk[1])) }
        last = m.index + tok.length
      }
      if (last < s.length) parts.push.apply(parts, autolinkText(s.slice(last), key))
      return parts
    }
    // ---- pipe tables ----
    // Split a table row into trimmed cells; tolerates missing outer pipes.
    ...
    // Delimiter row: cells only made of '-', ':' and spaces (e.g. `| :---: | ---: |`).
    ...
    function parseTable(lines, start) {
      // start = header line index; start+1 = delimiter row.
      var aligns = splitRow(lines[start + 1]).map(function (c) {
        var left = c.slice(0, 1) === ':'
        var right = c.slice(-1) === ':'
      ...
      return { headers: headers, aligns: aligns, rows: rows, next: i }
    }
    function renderTable(b, i) {
      var alignStyle = function (j) { return b.aligns[j] ? { textAlign: b.aligns[j] } : null }
      return h('table', { className: 'dsh-pf-table', key: i },
        h('thead', null, h('tr', null, b.headers.map(function (c, j) {
          return h('th', { key: j, style: alignStyle(j) }, renderInline(c, 'th' + i + '_' + j))
        }))),
        h('tbody', null, b.rows.map(function (r, j) {
          return h('tr', { key: j }, r.map(function (c, k) {
            return h('td', { key: k, style: alignStyle(k) }, renderInline(c, 'td' + i + '_' + j + '_' + k))
          }))
        })))
    }
    function renderBlock(b, i) {
      if (b.t === 'h') return h('h' + Math.min(b.lvl, 6), { key: i, className: 'dsh-pf-md-h' }, renderInline(b.s, 'h' + i))
      if (b.t === 'quote') return h('blockquote', { key: i }, renderBlocks(parseBlocks(b.raw)))
      if (b.t === 'list') return h(b.ordered ? 'ol' : 'ul', { key: i }, b.items.map(function (it, j) {
        var kids = []
        if (it.task) kids.push(h('span', { key: 'tk' }, it.done ? '\u2611 ' : '\u2610 '))
        kids.push.apply(kids, renderInline(it.s, 'li' + i + '_' + j))
        if (it.sub && it.sub.length) kids.push.apply(kids, renderBlocks(parseBlocks(it.sub)))
        return h('li', { key: j, className: it.task && it.done ? 'dsh-pf-task-done' : null }, kids)
      }))
      if (b.t === 'code') return h('div', { className: 'dsh-pf-mdcode', key: i }, renderCode(b.s, b.lang || 'text'))
      if (b.t === 'hr') return h('hr', { key: i })
      if (b.t === 'table') return renderTable(b, i)
      return h('p', { key: i }, renderInline(b.s, 'p' + i))
    }
    function renderBlocks(blocks) { return blocks.map(renderBlock) }
    // Block parser: one pass over lines → block array. renderBlock renders it;
    // quotes and nested list items recurse back through parseBlocks.
    function parseBlocks(lines) {
      var blocks = [], list = null, code = null, quote = null, icode = null, para = []
      function flushPara() { if (para.length) { blocks.push({ t: 'p', s: para.join(' ') }); para = [] } }
      function flushList() { if (list) { blocks.push({ t: 'list', ordered: list.ordered, items: list.items }); list = null } }
      function flushQuote() { if (quote) { blocks.push({ t: 'quote', raw: quote }); quote = null } }
      function flushICode() {
        if (icode) {
          while (icode.length && !/\S/.test(icode[icode.length - 1])) icode.pop()
          blocks.push({ t: 'code', lang: 'text', s: icode.join('\n') })
          icode = null
        }
      }
      function indOf(s) { var m2 = /^([ \t]*)/.exec(s), n2 = 0, j2; for (j2 = 0; j2 < m2[1].length; j2++) n2 += m2[1].charAt(j2) === '\t' ? 4 : 1; return n2 }
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i]
        // fenced code: ``` or ~~~ (closing fence must match the opener)
        if (/^```/.test(line) || /^~~~/.test(line)) {
          var fch = line.slice(0, 3), flang = LANG_BY_EXT[line.slice(3).trim()] || line.slice(3).trim()
          flushPara(); flushList(); flushQuote(); flushICode()
          if (code && code.fence === fch) { blocks.push({ t: 'code', lang: code.lang, s: code.buf.join('\n') }); code = null }
          else if (!code) code = { lang: flang, buf: [], fence: fch }
          else code.buf.push(line)
          continue
        }
        if (code) { code.buf.push(line); continue }
        // indented (4-space/tab) code block: keep blank lines inside
        if (icode) {
          if (/^\s*$/.test(line)) { icode.push(''); continue }
          if (/^(?:    |\t)/.test(line)) { icode.push(line.replace(/^(?:    |\t)/, '')); continue }
          flushICode()
        }
        if (/^\s*$/.test(line)) { flushPara(); flushList(); flushQuote(); continue }
        // nested list content: line indented ≥2 spaces deeper than the open
        // list's base indent (relative depth — recursion handles deeper levels)
        if (list && indOf(line) >= list.base + 2) { list.items[list.items.length - 1].sub.push(line); continue }
        // start of an indented code block (non-list context only)
        if (!list && /^(?:    |\t)/.test(line)) { flushPara(); flushQuote(); icode = [line.replace(/^(?:    |\t)/, '')]; continue }
        var hm = /^(#{1,6})\s+(.*)$/.exec(line)
        if (hm) { flushPara(); flushList(); flushQuote(); blocks.push({ t: 'h', lvl: hm[1].length, s: hm[2] }); continue }
        var q = /^>\s?(.*)$/.exec(line)
        if (q) { flushPara(); flushList(); if (!quote) quote = []; quote.push(q[1]); continue }
        var lm = /^(\s*)([-*+]|\d+[.)])\s+(.*)$/.exec(line)
        if (lm) {
          flushPara(); flushQuote()
          var isOrd = /\d/.test(lm[2].charAt(0))
          if (!list || list.ordered !== isOrd) { flushList(); list = { ordered: isOrd, items: [], base: indOf(line) } }
          var it = { s: lm[3], task: false, done: false, sub: [] }
          var tk = /^\[([ xX])\]\s+(.*)$/.exec(it.s)
          if (tk) { it.task = true; it.done = tk[1] !== ' '; it.s = tk[2] }
          list.items.push(it)
          continue
        }
        if (/^-{3,}$/.test(line.trim())) { flushPara(); flushList(); flushQuote(); blocks.push({ t: 'hr' }); continue }
        // pipe table: current line has a '|' and the next line is a delimiter
        // row (only recognized at line-start context — code fences, quotes and
        // list lines are consumed by the checks above).
        if (line.indexOf('|') >= 0 && i + 1 < lines.length && isDelimRow(lines[i + 1])) {
          flushPara(); flushList(); flushQuote()
          var tb = parseTable(lines, i)
          blocks.push({ t: 'table', headers: tb.headers, aligns: tb.aligns, rows: tb.rows })
          i = tb.next - 1
          continue
        }
        flushList(); flushQuote()
        para.push(line.trim())
        // Setext heading: single pending paragraph line followed by =+ or ---+.
        // `---` only upgrades here — a standalone `---` (no pending para) stays hr.
        var nxt = i + 1 < lines.length ? lines[i + 1] : null
        if (para.length === 1 && nxt !== null && /^=+\s*$/.test(nxt)) { blocks.push({ t: 'h', lvl: 1, s: para[0] }); para = []; i++; continue }
        if (para.length === 1 && nxt !== null && /^-{3,}\s*$/.test(nxt)) { blocks.push({ t: 'h', lvl: 2, s: para[0] }); para = []; i++; continue }
      }
      flushPara(); flushList(); flushQuote(); flushICode()
      if (code) blocks.push({ t: 'code', lang: code.lang, s: code.buf.join('\n') })
      return blocks
    }
    function renderMarkdown(src) {
      // CRLF/CR line endings break every `.*$` line regex (JS `.` does NOT match
      // `\r`), so headings/lists/quotes silently degrade to literal text while
      // tables (which .trim() the row) still render. Normalize to LF first.
      return h('div', { className: 'dsh-pf-md' }, renderBlocks(parseBlocks(String(src).replace(/\r\n?/g, '\n').split('\n'))))
    }
...
.dsh-pf-iconbtn { display: flex; align-items: center; justify-content: center;
  width: 34px; height: 34px; border: none; border-radius: 5px; background: transparent;
  color: var(--dsw-alias-label-secondary, #6b7280); cursor: pointer; }
.dsh-pf-iconbtn svg { display: block; width: 16px; height: 16px; flex: none; }
...
/* Left-edge drag handle: resize the docked panel width. The panel is
   right-anchored, so dragging left grows the width. Hidden while maximized
   (already full) and on mobile (already full-width). */
.dsh-pf-resizer { position: absolute; left: 0; top: 0; bottom: 0; width: 6px;
  cursor: col-resize; z-index: 2; }
.dsh-pf-resizer:hover { background: var(--dsw-alias-brand-primary, #2563eb); opacity: .22; }
.dsh-pf-dl { display: inline-flex; align-items: center; justify-content: center;
  width: 34px; height: 34px; border-radius: 5px; text-decoration: none; flex: none; line-height: 0;
  color: var(--dsw-alias-label-secondary, #6b7280); cursor: pointer; }
.dsh-pf-dl svg { display: block; width: 16px; height: 16px; flex: none; }
...
.dsh-pf-md ul, .dsh-pf-md ol { margin: 6px 0; padding-left: 20px; }
.dsh-pf-md li { margin: 2px 0; }
.dsh-pf-md ul ul, .dsh-pf-md ul ol, .dsh-pf-md ol ul, .dsh-pf-md ol ol { margin: 2px 0; }
.dsh-pf-md li.dsh-pf-task-done { color: var(--dsw-alias-label-secondary, #6b7280); }
...
.dsh-pf-table { border-collapse: collapse; margin: 8px 0; }
.dsh-pf-table th, .dsh-pf-table td { border: 1px solid #d0d7de; padding: 4px 10px; }
.dsh-pf-table th { background: #f6f8fa; }
.dsh-pf-mdcode { margin: 8px 0; }
/* Light "editor" background for the code/preview/markdown content so the
   light-theme highlight colors read clearly (don't inherit the theme's
   possibly gray/dark overlay). */
.dsh-pf-preview, .dsh-pf-textbox, .dsh-pf-md { background-color: #fff; }
.dsh-pf-pre { background-color: #f6f8fa; color: #24292f; }
.dsh-pf-md { color: #24292f; }
@media (max-width: 720px) {
  .dsh-pf { left: 0 !important; width: 100vw !important; top: 0; bottom: 0; height: auto; border-left: none;
    border-top: 1px solid var(--dsw-alias-border-l1, #e5e7eb); }
  /* Mobile already fills the viewport, so a drag handle is pointless. */
  .dsh-pf-resizer { display: none; }
}
`
    ...
    // Stroke icon set (16×16 on a 24 viewBox), same visual language as FolderIcon.
    function svgIcon(dPaths) {
      return h('svg', { width: 16, height: 16, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' },
        dPaths.map(function (d, i) { return h('path', { key: i, d: d }) }))
    }
    function IconBack() { return svgIcon(['M19 12H5', 'M12 19l-7-7 7-7']) }
    function IconClose() { return svgIcon(['M18 6L6 18', 'M6 6l12 12']) }
    function IconRefresh() { return svgIcon(['M23 4v6h-6', 'M1 20v-6h6', 'M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15']) }
    function IconDownload() { return svgIcon(['M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4', 'M7 10l5 5 5-5', 'M12 15V3']) }
    function IconMaximize() { return svgIcon(['M8 3H5a2 2 0 0 0-2 2v3', 'M16 3h3a2 2 0 0 1 2 2v3', 'M8 21H5a2 2 0 0 1-2-2v-3', 'M16 21h3a2 2 0 0 0 2-2v-3']) }
    function IconRestore() { return svgIcon(['M8 3H5a2 2 0 0 0-2 2v3', 'M16 3h3a2 2 0 0 1 2 2v3', 'M8 21H5a2 2 0 0 1-2-2v-3', 'M16 21h3a2 2 0 0 0 2-2v-3', 'M4 12h16']) }

    // ---------- Panel ----------
    // Panel size persistence: the docked (not-maximized) width and the maximized
    // flag are stored in localStorage so the panel reopens the way you left it.
    var PF_W_KEY = 'dsh-pf:w'
    var PF_M_KEY = 'dsh-pf:m'
    var PF_DEFAULT_W = 560
    var PF_MIN_W = 300
    function pfRead(key, fallback) {
      try { var v = window.localStorage.getItem(key); if (v != null) return v } catch (e) {}
      return fallback
    }
    function pfWrite(key, val) {
    ...
    function Panel(props) {
      ...
      // ---- panel size: docked width + maximized flag, persisted to localStorage ----
      var widthState = React.useState(function () {
        var n = parseInt(pfRead(PF_W_KEY, ''), 10)
        return (n >= PF_MIN_W) ? n : PF_DEFAULT_W
      })
      var width = widthState[0]
      var setWidth = widthState[1]
      var maxState = React.useState(function () { return pfRead(PF_M_KEY, '0') === '1' })
      var maximized = maxState[0]
      var setMaximized = maxState[1]
      var toggleMax = function () {
        setMaximized(function (m) { var n = !m; pfWrite(PF_M_KEY, n ? '1' : '0'); return n })
      }
      // Drag the left edge to resize the docked width. Panel is right-anchored
      // so newWidth = viewportWidth - clientX. Clamped to [min, near-full]; the
      // settled value is persisted on mouseup.
      ...
      var openEntry = function (entry) {
        ...
        if (mode === 'text') {
          ...
          // Fetch the PDF as a blob and use a same-origin object URL so the
          // iframe renders it inline (the direct /file URL can trigger a
          // download/"Open in new tab" prompt, especially via a gateway).
          setPreview(Object.assign({}, base, { loading: true, error: '' }))
          fetch('/plugins/project-files/file?path=' + encodeURIComponent(entry.path))
            .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.blob() })
            .then(function (b) { setPreview(Object.assign({}, base, { mode: 'pdf', src: URL.createObjectURL(b) })) })
            .catch(function (e) { setPreview(Object.assign({}, base, { error: String(e) })) })
        } else if (mode === 'html') {
          // Same-origin blob + iframe so the page renders inline instead of a
          // download prompt. Force type text/html on the blob: the /file route
          // (index.js) now serves text/html, but if the running host has not
          // reloaded it still returns application/octet-stream (or a gateway
          // rewrites the type), which makes the browser DOWNLOAD instead of
          // rendering. Re-typing guarantees the iframe displays the page.
          setPreview(Object.assign({}, base, { loading: true, error: '' }))
          fetch('/plugins/project-files/file?path=' + encodeURIComponent(entry.path))
            .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.blob() })
            .then(function (b) {
              var html = new Blob([b], { type: 'text/html' })
              setPreview(Object.assign({}, base, { mode: 'html', src: URL.createObjectURL(html) }))
            })
            .catch(function (e) { setPreview(Object.assign({}, base, { error: String(e) })) })
        } else {
          // image / video / audio stream inline via /file; unknown shows a
            ...
            sorted.length === 0 ? h('div', { className: 'dsh-pf-note' }, t('empty')) :
              sorted.map(function (entry) {
                return h('div', {
                  ...
                  entry.type === 'file' ? h('a', { className: 'dsh-pf-dl', title: t('download'), href: dlUrl(entry), download: true, onClick: function (e) { e.stopPropagation() } }, h(IconDownload)) : null)
              }))
      ...
      var previewEl = (function () {
        ...
        if (preview.mode === 'pdf') {
          if (preview.src) return h('iframe', { className: 'dsh-pf-frame', src: preview.src })
          if (preview.loading) return h('div', { className: 'dsh-pf-loading' }, t('loading'))
          return h('div', { className: 'dsh-pf-note' }, t('notPreviewable'))
        }
      ...
      // Panel geometry: maximized fills the viewport; otherwise the docked width
      // (user-resized, persisted). The left-edge handle drags to resize it.
      var panelStyle = maximized ? { left: 0, width: '100vw' } : { width: width + 'px' }
      var resizer = !maximized ? h('div', { className: 'dsh-pf-resizer', 'data-resizer': '1', title: t('resize'), onMouseDown: startResize }) : null
      var maxBtn = h('button', {
        className: 'dsh-pf-iconbtn',
        title: maximized ? t('restore') : t('maximize'),
        'aria-label': maximized ? t('restore') : t('maximize'),
        'data-maximize': maximized ? 'restore' : 'max',
        onClick: toggleMax,
      }, maximized ? h(IconRestore) : h(IconMaximize))

      if (preview) {
        // Preview page: full area, with back + download + close.
        return h('div', { className: 'dsh-pf', style: panelStyle },
          resizer,
          h('div', { className: 'dsh-pf-header' },
            h('button', { className: 'dsh-pf-iconbtn', title: t('back'), onClick: function () { setPreview(null) } }, h(IconBack)),
            h('span', { className: 'dsh-pf-title' }, preview.name || t('preview')),
            maxBtn,
            h('a', { className: 'dsh-pf-iconbtn dsh-pf-dl', title: t('download'), href: dlUrl(preview), download: true }, h(IconDownload)),
            h('button', { className: 'dsh-pf-iconbtn', title: t('close'), onClick: onClose }, h(IconClose))),
      ...
      return h('div', { className: 'dsh-pf', style: panelStyle },
        ...
        h('div', { className: 'dsh-pf-header' },
          ...
          maxBtn,
          h('button', { className: 'dsh-pf-iconbtn', title: t('refresh'), onClick: function () { loadDir(cwd) } }, h(IconRefresh)),
          h('button', { className: 'dsh-pf-iconbtn', title: t('close'), onClick: onClose }, h(IconClose))),
        crumbsEl,
        treeEl)
    }
    ...
    function ProjectFilesButton(props) {
      ...
      // Click-outside: while the panel is open, a mousedown that lands neither
      // inside the panel (.dsh-pf) nor on the toggle button itself closes it.
      // mousedown (not click) avoids racing with clicks bubbling inside the
      // panel; the button check keeps the opening click from instantly closing.
      React.useEffect(function () {
        if (!open) return
        var onDown = function (e) {
          var target = e.target
          if (target && target.closest && target.closest('.dsh-pf')) return
